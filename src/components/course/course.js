...
const Course = (props) => {
    console.log(props)
    const [pdf, set_pdf] = useState(false)

    const data = [{name:'History','sub':"Indian Tradition"},{name:'Math',sub:'Prime Number'}]
    const sub = (item) => {
        return(
        ...
        <Card.Title>{item.name}</Card.Title>
              ...
              <Button variant="primary" onClick={()=>set_pdf(true)}>Read</Button>
            </Card.Body>
          </Card>
    ...
    return(
        <div style={{}}>
            <Row>
                {
                    pdf ?  <PDFViewer style={{height:'300vh',width:'400vh'}}>
                    <Quixote />
                  </PDFViewer>
                      :data.map(item=>(sub(item)))
